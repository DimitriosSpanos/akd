...
from helper.loops import train_distill,train_distill_with_AKD, validate
...
def parse_option():
    ...
    # distillation
    parser.add_argument('--distill', type=str, default='AKD', choices=['AKD', 'AKD_CRD'])
    parser.add_argument('--trial', type=str, default='1', help='trial tag')
    parser.add_argument('--l_AKD', type=float, default=10.0, help='the hyper-parameter /lambda_AKD of AKD, weighting the AKD loss')
    parser.add_argument('--l_1', type=float, default=1.0, help='the hyper-parameter /lambda_1 of AKD, weighting the L_1 loss')
    parser.add_argument('--l_2', type=float, default=0.1, help='the hyper-parameter /lambda_2 of AKD, weighting the L_2 and L_3 losses')
    ...
    parser.add_argument('-a', '--alpha', type=float, default=None, help='the hyper-parameter /beta of NORM, weighting the vanilla KD loss defined as the KL divergence between the teacher and student logits.')
    parser.add_argument('-b', '--beta', type=float, default=1.0, help=' the hyper-parameter /alpha of NORM, weighting the NORM loss')
    ...
    parser.add_argument('-s', '--co-sponge', type=int, default=4, help='the hyper-parameter of NORM')
...
def main():
    ...
    model_t.load_state_dict(torch.load(opt.path_t, weights_only=False)['model'])
    print('==> done')
    ...
    # -------- Anchor based Knowledge Distillation  --------
    if opt.distill == 'AKD' or opt.distill=='AKD_CRD':
        from helper.akd import calculate_anchor_set, Anchor_Net
        anchor_set = calculate_anchor_set(model_t, train_loader, subset_length="max", num_classes=opt.num_cls,
                                          anchors_per_class=1, method=opt.distill).cuda()
        print(f"Anchor set created with {anchor_set.size(0)} anchors")
        anchor_net = Anchor_Net(anchor_batch_size=anchor_set.size(0)).cuda()
        count_parameters(anchor_net)
        anchor_net.train()
        optimizer_anchor = optim.SGD(anchor_net.parameters(),lr=1e-3, weight_decay=1e-5)

        with torch.no_grad():
            _, _, a_feat_t = model_t(anchor_set, is_feat=True, preact=False)
            a_feat_t = a_feat_t.detach()

    optimizer = optim.SGD(trainable_list.parameters(), lr=opt.learning_rate, momentum=opt.momentum,
    ...
    print(f"==> Training with Anchor-based Knowledge Distillation ({opt.distill})")
    for epoch in range(init_epoch, opt.epochs + 1):
        time1 = time.time()
        adjust_learning_rate(epoch, opt, optimizer, len(train_loader))

        train_acc, train_loss, train_loss_ce, train_loss_norm = train_distill_with_AKD(anchor_set, anchor_net,
                            epoch, train_loader, module_list, criterion_list, optimizer, optimizer_anchor, opt, a_feat_t)
        time2 = time.time()
